...
func (d *Datum) ConvertTo(sc *stmtctx.StatementContext, target *FieldType) (Datum, error) {
	...
	case mysql.TypeYear:
		return d.convertToMysqlYear(sc, target, false)
	case mysql.TypeEnum:
...
// ConvertToMysqlYear converts datum into MySQL year ignoring overflow for the year type
func ConvertToMysqlYear(sc *stmtctx.StatementContext, d Datum) (Datum, error) {
	return d.convertToMysqlYear(sc, types.NewFieldType(mysql.TypeYear), true)
}

func (d *Datum) convertToMysqlYear(sc *stmtctx.StatementContext, target *FieldType, ignoreYearOverflow bool) (Datum, error) {
	var (
		ret    Datum
		y      int64
		err    error
	...
	y, err = AdjustYear(y, adjust, ignoreYearOverflow)
	ret.SetInt64(y)
	return ret, errors.Trace(err)
}
